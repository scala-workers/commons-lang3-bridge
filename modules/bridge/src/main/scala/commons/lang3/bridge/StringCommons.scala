...
class StringCommons[T: StrToOpt](value: T) {
  ...
  private object utils {
    def strToOpt[U: StrToOpt](t: U): Option[String] = {
      val mapping = TypeMapping.getMapping[StrToOpt, U]
      mapping.input(t).fold(Option(_), identity)
    }

    def tranCharSeqOptFunc(seq: Seq[Option[Char]]): Seq[Char] = seq.filter(_.isDefined).map(_.get)
    val tranCharSeqOpt                                        = tranCharSeqOptFunc _

    def tranCharSeqSeqOptFunc(seq: Seq[Option[CharSequence]]): Seq[CharSequence] = seq.map(_.orNull)
    val tranCharSeqSeqOpt                                                        = tranCharSeqSeqOptFunc _
  }

  private def strOpt: Option[String] = utils.strToOpt(value)
  ...
  def abbreviate[Abb: StrToOpt](abbrevMarker: Abb, maxWidth: Int): Option[String] =
    Option(Strings.abbreviate(strOpt.orNull, utils.strToOpt(abbrevMarker).orNull, maxWidth))
  ...
  def abbreviate[Abb: StrToOpt](abbrevMarker: Abb, offset: Int, maxWidth: Int): Option[String] =
    Option(Strings.abbreviate(strOpt.orNull, utils.strToOpt(abbrevMarker).orNull, offset, maxWidth))
  ...
  def abbreviateMiddle[M: StrToOpt](middle: M, length: Int): Option[String] =
    Option(Strings.abbreviateMiddle(strOpt.orNull, utils.strToOpt(middle).orNull, length))
  ...
  def appendIfMissing[S: StrToOpt](suffix: S, suffixes: CharSequence*): Option[String] =
    Option(Strings.appendIfMissing(strOpt.orNull, utils.strToOpt(suffix).orNull, suffixes: _*))
  ...
  def appendIfMissingIgnoreCase[S: StrToOpt](suffix: S, suffixes: CharSequence*): Option[String] =
    Option(Strings.appendIfMissingIgnoreCase(strOpt.orNull, utils.strToOpt(suffix).orNull, suffixes: _*))
  ...
  def center[P: StrToOpt](size: Int, padStr: P): Option[String] =
    Option(Strings.center(strOpt.orNull, size, utils.strToOpt(padStr).orNull))
  ...
  /** <p>Compare two Strings lexicographically, as per {@link String# compareTo ( String )}, returning :</p> <ul> <li>{@code int = 0}, if
    ...
    */
  def compare[O: StrToOpt](other: O): Int = Strings.compare(strOpt.orNull, utils.strToOpt(other).orNull)
  ...
  def compare[O: StrToOpt](other: O, nullIsNull: Boolean): Int =
    Strings.compare(strOpt.orNull, utils.strToOpt(other).orNull, nullIsNull)
  ...
  def compareIgnoreCase[O: StrToOpt](other: O): Int =
    Strings.compareIgnoreCase(strOpt.orNull, utils.strToOpt(other).orNull)
  ...
  def compareIgnoreCase[O: StrToOpt](other: O, nullIsLess: Boolean): Int =
    Strings.compareIgnoreCase(strOpt.orNull, utils.strToOpt(other).orNull, nullIsLess)
  ...
  def contains[To: StrToOpt](searchSeq: To): Boolean =
    Strings.contains(strOpt.orNull, utils.strToOpt(searchSeq).orNull)
  ...
  def containsAny[S: VarArgsOfCharOrString](searchArgs: S*)(implicit tt: ClassTag[S]): Boolean = {
    ...
    mapping
      ...
      .fold(
        ...
        utils.tranCharSeqOpt.andThen(dealWithSeqChar),
        utils.tranCharSeqSeqOpt.andThen(dealWithSeqCharSequence)
      )
  }
